...
class Logidit_Admin {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->logidit_options = get_option($this->plugin_name);
	...
	public function enqueue_styles() {
		 ...
		 * defined in Logidit_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Logidit_Loader will then create the relationship
		...
		if ( 'settings_page_logidit' == get_current_screen() -> id ) {
             // CSS stylesheet for Color Picker
			wp_enqueue_style( 'wp-color-picker' );            
			wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/logidit-admin.css', array('wp-color-picker'), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		if ( 'settings_page_logidit' == get_current_screen() -> id ) {
			wp_enqueue_media();   
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/logidit-admin.js', array('jquery', 'wp-color-picker', 'media-upload'), $this->version, false );
		}
	}
...
public function add_plugin_admin_menu() {
     ...
     *
     */
    add_options_page( 'Logidit - Editor del Area de Login ',
    	'Logidit',
    	'manage_options',
    	$this->plugin_name,
...
public function display_plugin_setup_page() {
	include_once( 'partials/logidit-admin-display.php' );
}
...
/************************************LOGIN************************************/
  ...
  /**
     ...
     * @since    1.0.0
     */
  private function logidit_login_logo_css(){
  	if(isset($this->logidit_options['login_logo_id']) && !empty($this->logidit_options['login_logo_id'])){

  		$login_logo = wp_get_attachment_image_src($this->logidit_options['login_logo_id'], 'thumbnail');
  ...
  private function logidit_login_background_color(){
  	if(isset($this->logidit_options['login_background_color']) && !empty($this->logidit_options['login_background_color']) ){
  		$background_color_css  = "body.login{ background:".$this->logidit_options['login_background_color']."!important;}";
  ...
  private function logidit_login_button_color(){
  	if(isset($this->logidit_options['login_button_primary_color']) && !empty($this->logidit_options['login_button_primary_color']) ){

  		$button_color = $this->logidit_options['login_button_primary_color'];
  ...
  public function logidit_login_css(){
  	if( !empty($this->logidit_options['login_logo_id']) || $this->logidit_login_background_color() != null || $this->logidit_login_button_color() != null){
  		echo '<style>';
  		if( !empty($this->logidit_options['login_logo_id'])){
  			echo $this->logidit_login_logo_css();
  		}
  		if($this->logidit_login_background_color() != null){
  			echo $this->logidit_login_background_color();
  		}
  		if($this->logidit_login_button_color() != null){
  			echo $this->logidit_login_button_color();
  		}
  		echo '</style>';
  	}
  }
